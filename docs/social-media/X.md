**30-Day X Growth Test Plan for Your AI/ML Agent Platform (2026 Meta – Pure Organic, No Money Spent)**

This is your exact, follow-to-the-letter playbook. It’s built for a **new, unverified account** like yours: open-source AI/ML projects + real build-in-public videos + meme images paired with useful teaching posts.  

No hacks. No automation. No engagement pods. No Premium yet. Just what the 2026 algorithm rewards right now: **high-dwell-time native content, real conversations, semantic relevance, and consistent value from a credible builder**.  

Follow this exactly for 30 days. Track results daily (X Analytics → Impressions, Profile visits, Engagements). At day 30 we’ll review and adjust. You will see traction if you ship real work and stay consistent.

---

### **Day 0: Profile Setup (Do this first – 30 minutes max)**

Your profile is the #1 credibility signal for the algo when you have zero followers.

- **Profile Pic**: Professional but human (you coding or with your agent screen).
- **Header Image**: Clean screenshot of your agent platform (or simple text: “Rebuilding real AI agents in public | Open source everything”).
- **Bio** (copy-paste exactly):  
  `Engineer rebuilding a real AI agent platform from scratch in public. Open-sourcing the code + teaching what actually breaks. No fluff.`
- **Location**: Add “Building in public”
- **Link**: Leave empty for now (we add later).
- **Pinned Post**: We’ll create this on Day 1.
- **Turn on notifications** for 10–15 big accounts in AI/ML/open-source (search: “AI agents”, “open source AI”, “ML engineering”, builders with 10k+ followers).

**Do NOT** change anything after today. Consistency in voice = algo trust.

---

### **Your Positioning (Say this in every post until it feels natural)**

You are: **The engineer rebuilding a production-grade AI agent platform in public – showing the messy reality + open-sourcing it.**

Never call yourself a “content creator” or “AI educator.” You are a builder who teaches from pain.

---

### **Content Pillars (Every post must hit at least one)**

1. **Build in Public** – Raw progress, what broke today, decisions made.
2. **Deep Teaching** – Explain hard things simply (state management, memory, tool calling, etc.).
3. **Meme + Insight** – Your meme image + 1 clear lesson (this is your virality engine).

All content must feel native to X: text-first, rich media (your memes + native videos).

---

### **Daily Posting Cadence (Follow exactly – this is what new accounts need in 2026)**

- **3 original posts per day** (space them 2–4 hours apart, e.g., 8am, 12pm, 5pm your local time).
- **10–15 thoughtful replies per day** to bigger accounts in the AI/ML/open-source niche (this is 80% of your growth engine).
- **1 native video per day** (30–90 seconds max) – upload directly to X, never link YouTube.
- **Weekly**: 2 threads (one mid-week, one weekend).

Total time: ~60–90 minutes/day once you have the rhythm.

**Post formats that win right now** (use these templates):

**Type 1: Meme + Insight (your secret weapon – do 1/day)**  
[Upload your meme image first]  
Text:  
“Most AI agents die here → [meme]  

Here’s what I learned rebuilding mine this week:  
[1-sentence hard truth]  

Thread if you want the full breakdown →”

**Type 2: Build-in-Public Video Post (1/day)**  
Strong text hook + native video upload:  
“I just deleted 40% of my agent’s memory system because it was lying to itself in production.  

Watch the exact refactor (raw screen record, no editing):”  
[Upload 30–60s video of you coding + explaining]  
“Full code diff in reply if this hits 50 likes.”

**Type 3: Quick Insight Post (1/day)**  
Short, punchy text:  
“Hook sentence.  

One clear lesson from today’s build.  

[Optional: code snippet screenshot or diagram]”

**Thread Formula (2x/week)**  
1. Strong hook  
2. What I built today  
3. What broke  
4. What I changed  
5. Screenshot / code / video clip  
6. Soft CTA: “Building this live – follow for the next update”

---

### **Reply Strategy (This is how you get seen by real audiences – do this daily)**

Spend 20–30 minutes scrolling the AI/ML niche. Reply **only** to posts from accounts with 5k+ followers.

Rules:
- Add **real value** or your parallel experience from your agent build.  
- Never “Great post!” or emojis only.  
- Example reply:  
  “This is exactly where my agent broke last week – state became inconsistent across tools. I fixed it by [your 1-sentence solution]. Happened because [quick why]. Curious if you’ve seen the same at scale?”

Reply to **every single reply** on your own posts within the first hour. This is a massive algo signal.

---

### **Link & Self-Promo Rules (Critical – do not break these)**

- **Never** put GitHub, YouTube, or project links in the main post for the first 30 days.  
- Value-first only.  
- When you have strong engagement (50+ likes or 10+ replies):  
  → Add link in the **first reply** to your post, or in a follow-up post 10–20 minutes later.  
- First project mention: Day 15 at earliest, only after you have 100+ impressions on a post.

---

### **30-Day Schedule (Follow to the letter)**

**Days 1–7: Foundation Week**  
- 3 posts/day (mix of meme + video + insight)  
- 10 replies/day  
- Pin your first “Why I’m doing this” post (use the example below)  
- Goal: 50–100 profile visits, consistent posting habit

**Days 8–14: Momentum Week**  
- Same cadence  
- Add your first thread  
- Start tagging your project name lightly in replies only  
- Goal: First 1–2 posts > 500 impressions

**Days 15–21: Acceleration Week**  
- Introduce project name naturally  
- 1 video + 1 meme daily minimum  
- Reply volume up to 15/day  
- Goal: First real conversation threads

**Days 22–30: Flywheel Week**  
- Turn one video into a thread + meme version  
- Start soft project teases (still no links in main post)  
- Goal: Clear upward trend in impressions/followers

**Example First Pinned Post (copy-paste and post on Day 1)**  
“I’m rebuilding a real AI agent platform from scratch in public.  

Not a demo. Not a wrapper.  

Persistent state, actual tool execution, memory that doesn’t hallucinate, scheduling that survives production.  

Most agents break the second you leave the notebook. I’m showing exactly why + open-sourcing the fixes.  

Day 1 starts tomorrow. Follow if you want the unfiltered reality.”

---

### **Tracking & Rules You Must Follow**

Daily log (keep in a note):  
- Date  
- 3 posts + impressions each  
- Video watch %  
- Number of replies made  
- Profile visits  
- New followers  

**Never do** (instant throttle risk):  
- Mass replies or copy-paste  
- Any automation  
- Begging for follows/likes  
- More than 2 hashtags (use 0–1)  
- External links early  
- Generic AI fluff

**Your edge**: Your memes + real build videos + deep agent insights are perfect for 2026. The algo loves “messy progress from a real builder” + native video + visual memes.

Post your first 3 pieces today using the templates above.  

Reply here with:  
1. Your @handle  
2. Screenshot of your first post (or link)  

I’ll review it immediately and give exact tweaks so you don’t waste a single day.  

You’ve got the exact playbook. Now ship. Consistency beats everything in 2026. Let’s get you traction before you spend a dime.